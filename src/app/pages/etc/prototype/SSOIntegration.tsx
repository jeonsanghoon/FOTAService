...
export function EtcPrototypeSSOIntegration() {
  const steps = [
    { num: 1, title: '타시스템에서 FOTA 메뉴 진입', desc: '기존 업무 시스템(ERP, IoT 플랫폼 등)에 구성된 FOTA 메뉴를 클릭합니다.', icon: Menu },
    { num: 2, title: 'Cognito SSO 인증', desc: '이미 로그인된 경우 토큰으로 인증됩니다. 미로그인 시 타시스템 로그인 화면으로 이동 후 인증됩니다.', icon: Shield },
    { num: 3, title: 'FOTA 플랫폼 자동 로그인', desc: 'SSO 인증 완료 후 /etc 전용 콘솔로 진입합니다.', icon: LogIn },
  ];
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm sm:text-base text-gray-600">타시스템에 FOTA 메뉴를 두고, Cognito SSO로 /etc 전용 콘솔에 자동 로그인되는 프로세스입니다.</p>
      </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-blue-800">타시스템과 동일 Cognito를 사용하여 한 번 로그인하면 별도 로그인 없이 FOTA로 진입합니다.</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 md:gap-2">
          ...
          <div className="flex items-center gap-2 px-4 py-3 bg-green-50 border border-green-200 rounded-lg">
            ...
            <span className="font-medium text-gray-900">/etc 콘솔 자동 로그인</span>
          </div>
        </div>
        <p className="text-sm text-gray-500 mt-4">타시스템 → FOTA 메뉴 → Cognito SSO → /etc 콘솔</p>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <p className="text-gray-600 text-sm mb-4">자동 로그인된 상태로 /etc 프로토타입의 업데이트 관리·디바이스 목록·실시간 로그를 이용합니다.</p>
        <Link
          to="/etc/prototype"
